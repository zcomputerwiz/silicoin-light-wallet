...
class WalletRpcApi:
    ...
    async def create_offer_for_ids(self, request):
        ...
        if success:
            return {
                "offer": Offer.from_bytes(trade_record.offer).compress().hex(),
    ...
    async def get_offer(self, request: Dict):
        ...
        offer_value: Optional[str] = Offer.from_bytes(trade_record.offer).compress().hex() if file_contents else None
    ...
    async def get_all_offers(self, request: Dict):
        ...
        for trade in all_trades:
            ...
            if file_contents and offer_values is not None:
                offer_values.append(Offer.from_bytes(trade.offer).compress().hex())
