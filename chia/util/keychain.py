...
DEFAULT_USER = f"user-silicoin-{CURRENT_KEY_VERSION}"  # e.g. user-silicoin-1.8
DEFAULT_SERVICE = f"silicoin-{DEFAULT_USER}"  # e.g. silicoin-user-silicoin-1.8
...
def supports_keyring_passphrase() -> bool:
    return False or os.environ.get("SILICOIN_PASSPHRASE_SUPPORT", "").lower() in ["1", "true"]
...
def obtain_current_passphrase(prompt: str = DEFAULT_PASSPHRASE_PROMPT, use_passphrase_cache: bool = False) -> str:
    ...
    before failing.
    """
    from silicoin.cmds.passphrase_funcs import prompt_for_passphrase
