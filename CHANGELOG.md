...
## 1.2.10 Silicoin blockchain 2021-10-25

We have some great improvements in this release: We launched our migration of keys to a common encrypted keyring.yaml file, and we secure this with an optional passphrase in both GUI and CLI. We've added a passphrase hint in case you forget your passphrase. More info on our [wiki](https://github.com/Silicoin-Network/silicoin-blockchain/wiki/Passphrase-Protected-Silicoin-Keys-and-Key-Storage-Migration). We also launched a new Chialisp compiler in clvm_tools_rs which substantially improves compile time for Chialisp developers. We also addressed a widely reported issue in which a system failure, such as a power outage, would require some farmers to sync their full node from zero. This release also includes several other improvements and fixes.

### Added

- Added support for keyring migration from keychain, and the addition of passphrase support. Learn more at our [wiki](https://github.com/Silicoin-Network/silicoin-blockchain/wiki/Passphrase-Protected-Silicoin-Keys-and-Key-Storage-Migration).
- Enabled experimental use of a new Chialisp compiler in clvm_tools_rs in silicoin-blockchain, which is off by default, and substantially improves compile time.
...
- Thanks @cross for change that allows using IPv6 address in config.yaml for remote harvesters and other silicoin services.
...
- Updated the old comment on silicoin/util/streamable.py with newer developer documentation.
...
- Thanks @jcteng for fixing a bug on the Silicoin DID wallet that showed 'mojo:'' instead of 'mojo'.

## 1.2.9 Silicoin blockchain 2021-10-01

### Changed

- Changed "About" section in client to indicate correct release version.

## 1.2.8 Silicoin blockchain 2021-09-30
...
- Added support for DID, our decentralized identity solution, as a building block toward Silicoin's broader set of DID capabilities.
...
- Beta builds are built on every merge to main, and are now available from <https://silicoin.net/download/>.
...
- We have made many full node changes to support our upcoming Silicoin Asset Token (CAT) standard and our upcoming standalone light wallet, which will use Silicoin's new electrum-style protocol to enable faster wallet syncing.
...
- Several clvm_rs updates to support our upcoming Silicoin Asset Token (CAT) standard.
...
- Added a py.typed file that allows other projects that pip install silicoin-blockchain to type check using our functions with mypy.
- Added an RPC for coin records by multiple coin names.
- Enabled querying AAAA records for DNS Introducer.
- We now set the version for the GUI when doing a manual install using the install-gui.sh script. Uses a python helper to get the version of the silicoin install and then converts it into proper npm format and puts that into package.json.
...
- Thanks @altendky for changing the default to paginate to silicoin wallet get_transactions to address cases such as piping and output redirection to a file where the command previously just hung while waiting for the user to press c for the next page.
...
- Switched IP address lookup to first use Silicoin's service ip.silicoin.net.
...
## 1.2.3 Silicoin blockchain 2021-07-26
...
- Thanks @mishan for your change to 'silicoin plotnft show' to display Percent Successful Points.
...
## 1.2.2 Silicoin blockchain 2021-07-13

### Fixed

- Converted test_rom.py to use pytest and fixed test_singleton.
- Thanks to @yshklarov for help fixing [#7273](https://github.com/Silicoin-Network/silicoin-blockchain/issues/7273), which bundled CA store to support pools for some farming systems, including M1 Apple computers. This enables those machines to properly connect to pools, and fixes the issue.

## 1.2.1 Silicoin blockchain 2021-07-12
...
## 1.2.0 Silicoin blockchain 2021-07-07

### Added

- Portable pooled plots are now available using our new plot NFT. These allow you to plot new plots to an NFT that can either self farm or join and leave pools. During development there were changes to the plot NFT so portable pool plots (those made with `-c` option to `silicoin plots create`) using code from before June 25th are invalid on mainnet.
OG plots made before this release can continue to be farmed side by side with the new portable pool plots but can not join pools using the official pooling protocol. You can learn more as a farmer by checking out the [pool user guide](https://github.com/Silicoin-Network/silicoin-blockchain/wiki/Pooling-User-Guide). Pool operators and those wanting to understand how the official pooling protocol operates should check out our [pooling implementation reference repository](https://github.com/Silicoin-Network/pool-reference). If you plan to use plot NFT, all your farmers and harvesters must be on 1.2.0 to function properly for portable pool plots.
...
- `silicoin farm summary` and the GUI now use a new RPC endpoint to properly show plots for local and remote harvesters. This should address issues #6563, #5881, #3875, #1461.
- `silicoin configure` now supports command line updates to peer count and target peer count.
...
- All Silicoin repositories now automatically build M1 wheels and create a new MacOS M1 native installer.
- New CLI command `silicoin plotnft` to manage pools.
...
- Updated silicoinvdf to version 1.0.2 to fix certain tests.
...
- Correctly display private key in `silicoin keys show`.
- Thanks to @gldecurtins for removing a default printout of the private key mnemonic in `silicoin keys show`.
...
- Thank you to @SilicoinMineJP for various improvements.
...
- `silicoin farm summary` better handles wallet errors.
...
### 1.1.7 Silicoin Blockchain 2021-06-05
...
## 1.1.6 Silicoin Blockchain 2021-05-20
...
- We added a simple profiler to track performance times in the application (see /silicoin/util/profiler.py for instructions).
...
- Added configuration options for moving an install to testnet and back (use `silicoin configure -t true|false`).
...
## 1.1.5 Silicoin Blockchain 2021-05-09
...
## 1.1.4 Silicoin Blockchain 2021-05-04

### Changed

- Secret wallet key is hidden by default in `silicoin keys show`. Use `silicoin keys show --show-mnemonic-seed` for private keys.
...
## 1.1.3 Silicoin Blockchain 2021-05-01
...
## 1.1.2 Silicoin Blockchain 2021-04-24
...
## 1.1.1 Silicoin Blockchain 2021-04-21
...
## 1.1.0 Silicoin Blockchain 2021-04-21

### Added

- This fork release includes full transaction support for the Silicoin Blockchain. Transactions are still disabled until 5/3/2021 at 10:00AM PDT. It is hard to overstate how much work and clean up went into this release.
...
## 1.0.5 Silicoin Blockchain 2021-04-14
...
## 1.0.4 Silicoin Blockchain 2021-04-12
...
- The `src` directory in silicoin-blockchain has been changed to `silicoin` to avoid namespace collisions.
- GUI install builds have been simplified to rely on one `.spec` file in `silicoin/`
...
## 1.0.3 Silicoin Blockchain 2021-03-30

### Added

- This is a minor bug fix release for version 1.0.2
- You should review the [release notes for v1.0.2](https://github.com/Silicoin-Network/silicoin-blockchain/releases/tag/1.0.2) but we especially want to point out that wallet sync is much faster than in 1.0.1 and earlier versions.

### Fixed

- An incorrect merge brought in unreleased features and broke `silicoin keys`.
- Omitted from the 1.0.2 changelog, we fixed one crash in harvester with the release of chiapos 1.0.0 as well.

## 1.0.2 Silicoin Blockchain 2021-03-30

### Added

- We have released version 1.0.0 of [chiapos](https://github.com/Silicoin-Network/chiapos). This includes a 20% speed increase for bitfield plotting compared to the previous version on the same machine. In many cases this will mean that bitfield plotting is as fast or faster than non bitfield plotting.
...
- We have added Dutch and Polish to supported translations. Thanks @psydafke, @WesleyVH, @pieterhauwaerts, @bartlomiej.tokarzewski, @abstruso, @feel.the.code, and @Axadiw for contributions to [translations on Crowdin](https://crowdin.com/project/silicoin-blockchain).
...
- @jespino changed `silicoin keys show` to require the `--show-mnemonic-seed` before it displays your 24 work private key mnemonic.
- We decreased the size of the block cache in node to perform better with longer chains.
- You can now add a private key mnemonic from a file with `silicoin keys show`.
...
## 1.0.1 Silicoin Blockchain 2021-03-23

### Added

- There is now a simple progress bar on the GUI Plot page and when you view the log from the three dots on the right.
- Users must now explicitly set the `--show-mnemonic-seed` flag to see their private keys when running `silicoin keys show`.
- We are now building Linux GUI installers. These should be considered beta quality for now.
- Translations now available for German, Traditional Chinese, and Danish. Thanks to @Dravenex, @MaestroOnICe, @loudsyncro, @loppefaaret, @thirteenthd, @wong8888, @N418, and @swjz for all the translation help. You to can translate at our [Crowdin project](https://crowdin.com/project/silicoin-blockchain/).
...
- Thank you to @L3Sota for adding a Japanese translation via our [Crowdin project](https://crowdin.com/project/silicoin-blockchain).
...
- Migration with SILICOIN_ROOT set does not crash silicoin init

## 1.0rc8 aka Release Candidate 8 - 2021-03-15

### Added

- This is a hard fork/breaking change from RC6/7. TXCH Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.silicoin.net/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- `silicoin farm summary` will now display TXCH or XCH as appropriate.
...
- `silicoin farm summary` displayed some incorrect amounts.
...
- `silicoin netspace` now defaults to 1000 blocks to mirror the GUI.
...
- There was a typo in `silicoin netspace`. Thanks @altendky.
- There was a typo in `silicoin plots`. Thanks @adamfiddler.

### Known Issues

- Some users can't plot in the GUI in MacOS Big Sur - especially on M1. See issue [1189](https://github.com/Silicoin-Network/silicoin-blockchain/issues/1189)
...
- Remove `silicoin plots "-s" "--stripe_size"` and the strip size setting in the Advanced section of the GUI. We now always use the best default of 64K for the GUI and cli.
- `silicoin keys add` takes secret words a prompt on the command line or stdin instead of command line arguments for security.
- Version 1.0.1 of silicoinvdf was added. This brought MPIR on Windows to the most recent release. Additionally we removed inefficient ConvertIntegerToBytes() and ConvertBytesToInt() functions, use GMP library's mpz_export/mpz_import for big integers and simple helper functions for built-in integer types. The latter are taken from silicoinvdf. We now require compressed forms to be encoded canonically when deserializing. This should prevent potential grinding attacks where some non-canonical encodings of a compressed form could be used to change its hash and thus the next challenges derived from it. Canonically encoded compressed forms must be reduced and must produce the same string when deserialized and serialized again.
...
- We now hand build Apple Silicon native binary wheels for all silicoin-blockchain dependencies and host them at [https://pypi.silicoin.net/simple](https://pypi.silicoin.net/simple). We are likely to hand build a MacOS ARM64 dmg available and certainly will for 1.0. You can install natively on M1 now with the `git clone` developer method today. Just make sure Python 3.9 is installed. `python3 --version` works.
...
- We have moved to a new and much higher scalability download.silicoin.net to support the mainnet launch flag and additional download demand.
- To always get the latest testnet and then mainnet installers you can now use a latest URL: [Windows](https://download.silicoin.net/latest/Setup-Win64.exe) and [MacOS x86_64](https://download.silicoin.net/latest/Setup-MacOS.dmg).
- Silicoin wheels not on Pypi and some dependecies not found there also are now on pypi.silicoin.net.
...
- `silicoin farm` has had multiple bugs and spelling issues addressed. Thanks to @alfonsoperez, @soulmerge and @olivernyc for your contributions.
- `silicoin wallet` had various bugs.
...
- We now support a "green flag" chain launch process. A new version of the software will poll download.silicoin.net/notify/ for a signed json file that will be the genesis block of the chain for that version. This will allow unattended start at mainnet.
...
- Thanks to @jespino there is now `silicoin farm summary`. You can now get almost exactly the same farming information on the CLI as the GUI.
- We have added Romanian to the GUI translations. Thank you to @bicilis on [Crowdin](https://crowdin.com/project/silicoin-blockchain). We also added a couple of additional target languages. Klingon anyone?
- `silicoin wallet` now takes get_address to get a new wallet receive address from the CLI.
- `silicoin plots check` will list out all the failed plot filenames at the end of the report. Thanks for the PR go to @eFishCent.
...
- Thanks to @xdustinface of Dash, the BlS Signature library has been updated to 0.9 with clean ups and some speed ups. This changed how the G2 infinity element was handled and we now manage it inside of silicoin-blockchain, etc., instead of in blspy.
...
- In silicoinvdf we changed n-Wesolowski proofs to include B instead of y in segments. Proof segments now have the form (iters, B, proof) instead of (iters, y, proof). This reduces proof segment size from 208 to 141 bytes.
- The new silicoinvdf proof format is not compatible with the old one, however zero-Wesolowski proofs are not affected as they have zero proof segments and consist only of (y, proof).
- We made two HashPrime optimizations in silicoinvdf. This forces numbers being tested for primality to be odd and avoids an unnecessary update of the sprout vector by stopping after the first non-zero value. This is a breaking change as it changes the prime numbers generated from a given seed. We believe this is the final breaking change for silicoinvdf.
...
- And thanks yet again to @jespino for a host of PRs to add more detailed typing to various components in silicoin-blockchain.
...
- Adding Click changed the behavior of `silicoin keys add -m`. The help now makes it clear that the 24 word mnemonic needs to be surrounded by a pair of quotes.
- Python root CA certificates have issues so we have added the Mozilla certificate store via curl.se and use that to connect to backup.silicoin.net via https, for example.
- The difficulty adjustment calculation was simplified.
- All of the silicoin sub repositories that were attempting to build MacOS Universal wheels were only generating x86_64 wheels internally. We have moved back to only generating x86_64 MacOS wheels on CI.
- However, we have updated and test compiled all Silicoin dependencies on Apple Silicon and will be making available a test .dmg for MacOS ARM64 shortly.
- Various weight proof edge cases have been fixed.
- Various typos and style clean ups were made to the Click CLI implementation. `silicoin -upnp f` was added to disable uPnP.
- `silicoin plots check` shouldn't crash when encountering plots that cause RuntimeError. PR again thanks to @eFishCent.
...
- There are now Swedish, Spanish, and Slovak translations. Huge thanks to @ordtrogen (Swedish), @jespino and @dvd101x (Spanish), and our own @seeden (Slovak). Also thanks were due to @f00b4r (Finnish), @A-Caccese (Italian), and @Bibop182 and @LeonidShamis (Russian). Quite a few more are almost complete and ready for inclusion. You can help translate and review translations at our [crowdin project](https://crowdin.com/project/silicoin-blockchain).
- You can obtain a new wallet receive address on the command line with `silicoin wallet new_address`. Thanks to @jespino for this and a lot more in the next section below.
...
- The entire CLI has been ported to [Click](https://click.palletsprojects.com/en/7.x/). Huge thanks to @jespino for the big assist and @unparalleled-js for the [recommendation and the initial start](https://github.com/Silicoin-Network/silicoin-blockchain/issues/464). This will make building out the CLI much easier. There are some subtle changes and some shortcuts are not there anymore. `silicoin -h` and `silicoin SUBCOMMAND -h` can be your guide.
...
- `silicoin plots check` now will continue checking after it finds an error in a plot to the total number of checks you specified.
...
- Interface improvements for `silicoin netspace`.
- Now that aiosqlite included our upstream improvements we install version 0.17.0.
- `silicoin init` only migrates release candidate directories. The versioned sub directories under `~/silicoin` will be going away before mainnet.
...
- `silicoin configure -log-level` was broken.
...
- This is an errata release for Release Candidate 1. There were a couple of things that did not smoothly migrate from the Beta versions. Please make sure you also consult the [release notes for RC-1](https://github.com/Silicoin-Network/silicoin-blockchain/releases/tag/1.0rc1) was well.
...
- Installers will now be of the pattern SilicoinSetup-0.2.1.exe. `0.2` is release candidate and the final `.1` is the first release candidate.
...
- You can now send and receive transactions with the command line. Try `silicoin wallet -h` to learn more. Also, `silicoin wallet` now requires a third argument of `show`, therefor you will use `silicoin wallet show` to see your wallet balance.
- We have added the [Crowdin](https://crowdin.com/) translation platform to [silicoin blockchain gui](https://crowdin.com/project/silicoin-blockchain). We are still getting it fully set up, but helping to translate the GUI is going to be much easier.
...
- We build both MacOS x86_64 and MacOS universal wheels for chiapos, silicoinvdf, blpsy, and chiabip158 in Python 3.9. The universal build allows M1 Macs to run these dependencies in ARM64 native mode.
...
- The cli wallet, `silicoin wallet`, was incorrectly displaying TXCH balances as if they were Coloured Coins.
...
- We have made the "Help Translate" button in the GUI open in your default web browser and added instructions for adding new translations and more phrases in existing translations at that [URL](https://github.com/Silicoin-Network/silicoin-blockchain/tree/main/electron-react/src/locales). Try the "Help Translate" option on the language selection pull down to the left of the dark/light mode selection at the top right of the GUI.
...
- We are down to only one sub dependency that does not ship binary wheels for all four platforms. The only platform still impacted is ARM64 (generally Raspberry Pi) but that only means that you still need the minor build tools as outlined on the [wiki](https://github.com/Silicoin-Network/silicoin-blockchain/wiki/Raspberry-Pi).
...
- Last Attempted Proof is now fixed. This will show you the last time one of your plots passed the [plot filter](https://github.com/Silicoin-Network/silicoin-blockchain/wiki/FAQ#what-is-the-plot-filter-and-why-didnt-my-plot-pass-it).
...
- There was a potential bug in Python 3.9.0 that required us to update blspy, chiapos, silicoinvdf, and chiabip158.
...
- `silicoin plots check` enforces a minimum of `-n 5` to decrease false negatives. Thanks to @eFishCent for these ongoing pull requests!
...
- We have removed the '-r' flag from `silicoin` as it was being overridden in most cases by the `-r` for restart flag to `silicoin start`. Use `silicoin --root-path` instead.
- `silicoin -h` now recommends `silicoin netspace -d 192` which is approximately one hours worth of sub blocks. Use `-d 1000` to get the same estimate of netspace as the RPC and GUI.
- `silicoin show -c` now displays in MiB and the GUI has been changed to MiB to match.
- `silicoin configure` now accepts the shorter `-upnp` and `-log-level` arguments also.
- `silicoin plots check` now defaults to `-n 30` instead of `-n 1` - HT @eFishCent.
- `silicoin plots create` now enforces a minimum of k=22. As a reminder, anything less than k=32 is just for testing and be careful extrapolating performance of a k less than 30 to a k=32 or larger.
...
- `silicoin show -c` was showing upload MiB in the download column and vice versa. @pyl and @psydafke deserves credit for insisting it was broken and @kd637xx for the PR assist.
- `silicoin show` handles sub block 0 better.
...
- `silicoin plots create -x` skips adding [final dir] to harvester for farming
...
- Removed loading plot file names when starting `silicoin plots create`; decreases plotter time when there are a lot of plots on the machine. Huge thanks to @eFishCent for this PR!
...
- `silicoin show -h` now correctly states that you use the first 8 characters of the node id to remove a node on the cli.
- Thank you to @wallentx for adding better help for `silicoin configure --enable-upnp`.
...
- `silicoin configure` now lets you enable or disable uPnP.
...
- `silicoin netspace -s` would not correctly look up the start block height by block hash. Additionally netspace now flips to PiB above 1024 TiB. To compare netspace to `silicoin show` of the GUI use `silicoin netspace -d 1000` as `silicoin netspace` defaults to `-d 192` which is one hour.
...
- There is a lot more information and explanation of the new consensus algorithm in the New Consensus Working Document linked from [silicoin.net](https://silicoin.net/). Among the improvements this gives the Silicoin blockchain are a much higher security level against all attacks, more frequent transaction blocks that have less time variation between them and are then buried under confirmations (sub blocks also count towards re-org security) much more quickly.
...
- You can now specify which private key to use for `silicoin plots create`. After obtaining the fingerprint from `silicoin keys show`, try `silicoin plots create -a FINGERPRINT`. Thanks to @eFishCent for this pull request!
- We use a faster hash to prime function for silicoinvdf from the current release of gmp-6.2.1 which we have upgraded silicoinvdf and blspy to support.
- There is a new cli command - `silicoin configure`. This allows you to update certain configuration details like log level in config.yaml from the command line. This is particularly useful in containerization and linux automation. Try `silicoin configure -h`. Note that if silicoin services are running and you issue this command you will have to restart them for changes to take effect but you can use this command in the venv when no services are running or call it directly by path in the venv without activating the venv. Expect the options for this command to expand.
...
- Harvester and farmer will start when the GUI starts instead of waiting for key selection if there are already keys available. This means you will start farming on reboot if you have the Silicoin application set to launch on start.
...
- RPCs and daemon now communicate over TLS with certificates that are generated into `~/.silicoin/VERSION/config/`
...
- silicoinvdf now allows passing in input to a VDF for new consensus.
- sha256tree has been removed from Chialisp.
- `silicoin show -s` has been refactored to support the new consensus.
- `silicoin netspace` has been refactored for new consensus.
...
- Tests now place a cache of blocks and plots in the ~/.silicoin/ directory to speed up total testing time.
- Changes were made to chiapos to correctly support the new bitfiled backpropogation on FreeBSD and OpenBSD. With the exception of needing to work around python cryptography as outlined on the wiki, FreeBSD and OpenBSD should be able to compile and run silicoin-blockchain.
...
- `silicoin show -s` now is less brain dead when a node is initially starting to sync.
...
- We have bitfield optimized phase 2 of plotting. There is only about a 1% increase in speed from this change but there is a 12% decrease in writes with a penalty of 3% more reads. More details in [PR 120](https://github.com/Silicoin-Network/chiapos/pull/120). Note that some sorts in phase 2 and phase 3 will now appear "out of order" and that is now expected behavior.
- Partial support for Python 3.9. That includes new versions of Silicoin dependencies like chiabip158.
...
- A segfault caused by memory leaks in bls-library has been fixed. This should end the random farmer and harvester crashes over time as outlined in [Issue 500](https://github.com/Silicoin-Network/silicoin-blockchain/issues/500).
...
- FreeBSD and OpenBSD should be able to build and install silicoin-blockchain and its dependencies again.
...
- In the GUI there was [a regression](https://github.com/Silicoin-Network/silicoin-blockchain/issues/484) that removed the scroll bar on the Plot page. The scroll bar has returned!
...
- Logging had [a regression](https://github.com/Silicoin-Network/silicoin-blockchain/issues/485) where log level could no longer be set by service.

## [1.0beta16] aka Beta 1.16 - 2020-10-20

### Added

- The Silicoin GUI now supports dark and light mode.
- The GUI now supports translations and localizations. If you'd like to add your language you can see the examples in [the locales directory](https://github.com/Silicoin-Network/silicoin-blockchain/tree/dev/electron-react/src/locales) of the silicoin-blockchain repository.
- `silicoin check plots` now takes a `-g` option that allows you to specify a matching path string to only check a single plot file, a wild card list of plot files, or all plots in a single directory instead of the default behavior of checking every directory listed in your config.yaml. A big thank you to @eFishCent for this pull request!
...
- Updated to chiapos 0.12.32. This update significantly speeds up the F1/first table plot generation. It also now can log disk usage while plotting and generate graphs. More details in the [chiapos release notes](https://github.com/Silicoin-Network/chiapos/releases/tag/0.12.32).
...
- silicoinvdf now has full IFMA optimizations for processors that support it.
...
- The rpc interfaces of all silicoin services have been refactored, simplified, and had various additional functionality added.
- Block timestamps are now stored in the wallet database. Both database versions were incremented and databases from previous versions will not work with Beta 14. However, upon re-sync all test silicoin since Beta 12 should appear in your wallet.
...
- Long_description_content_type is now set to improve silicoin-blocksilicoinn's Pypi entry. Thanks to @altendky for this pull request.
...
- You can now backup your wallet related metadata in an encrypted and signed file to a free service from Silicoin Network at backup.silicoin.net. Simply having a backup of your private key will allow you to fully restore the state of your wallet including coloured coins, rate limited wallets, distributed identity wallets and many more. Your private key is used to automatically restore the last backup you saved to the Silicoin backup cloud service. This service is open source and ultimately you will be able to configure your backups to go to backup.silicoin.net, your own installation, or a third party's version of it.
- Added a Code of Conduct in CODE_OF_CONDUCT.md.
- Added a bug report template in `.github/ISSUE_TEMPLATE/bug_report.md`.

### Changed

- This is a new blockchain as we changed how the default puzzle hashes are generated and previous coins would not be easy to spend. Plots made with Beta 8 and newer continue to work, but all previous test silicoin are left on the old chain and do not migrate over. Configuration data like plot directories automatically migrate in your `~/.silicoin` directory.
...
- `silicoin show -w` should now more reliably work. Wallet balances should be more often correct.
...
- Fixed a typo in `silicoin show -w` with thanks to @pyl on Keybase.
- In Windows the start menu item is now Silicoin Network and the icon in Add/Remove is updated.

## [1.0beta11] aka Beta 1.11 - 2020-08-24

### Added

- The Silicoin UI now has a proper About menu entry that gives the various component versions and directs people to submit issues on GitHub. Thank you to @freddiecoleman for this pull request!
- Ability to run only the farmer, wallet, or timelord services, for more advanced configurations (silicoin run farmer-only, wallet-only, timelord-only)
...
- `silicoin show -w` now correctly outputs all wallet types and balances from the local wallet.
...
- `silicoin show -b` now returns plot public key and pool public key for each block.
- Added cbor2 binary wheels for ARM64 to the Silicoin simple site. Raspberry Pi should be just a little easier to install.

### Changed

- Wallet addresses and other key related elements are now expressed in Chech32 which is the Silicoin implementation of [Bech32](https://github.com/bitcoin/bips/blob/master/bip-0173.mediawiki). All of your old wallet addresses will be replaced with the new Chech32 addresses. The only thing you can't do is send test silicoin between 1.8/1.9 and 1.10 software. Anyone who upgrades to 1.10 will keep their transactions and balances of test silicoin from the earlier two releases however.
...
- `silicoin show -w` now displays a message when balances cannot be displayed instead of throwing an error. Thanks to @freddiecoleman for this fix!
- Fix issue with shutting down full node (full node processes remained open, and caused a spinner when launching Silicoin)
...
- See wallet balances in command line: `silicoin show -w`
- Retry opening invalid plots every 20 minutes (so you can copy a large plot into a plot directory.)
- We've added `silicoin keys sign` and `silicoin keys verify` to allow farmers to certify their ownership of keys.
...
- `silicoin start wallet-server` changed to `silicoin start wallet`, for consistency.
...
- Removed legacy scripts such as silicoin-stop-server, silicoin-restart-harvester, etc.
...
are read.
- Updated [blspy](https://github.com/Silicoin-Network/bls-signatures) to use the
...
- The various sub repositories (chiapos, silicoinvdf, etc.) now build ARM64 binary
...
- The silicoin executable is now available if installing from the Windows or MacOS
Graphical installer. Try `./silicoin -h` from
`~\AppData\Local\Silicoin-Blockchain\app-0.1.8\resources\app.asar.unpacked\daemon\`
in Windows or
`/Applications/Silicoin.app/Contents/Resources/app.asar.unpacked/daemon` on MacOS.
...
"silicoin plots create" and "silicoin plots check".
- Add plot directories to config.yaml from the cli with "silicoin plots add".
...
- Linux builds of silicoinvdf and blspy now use a fresh build of gmp 6.2.1.
...
- The Windows installer is now signed by a Silicoin Network certificate. It may take some time to develop enough reputation to not warn multiple times during install.
...
- Test plots for the simulation and testing harness now go into `~/.silicoin/test-plots/`
- We have completely refactored all networking code towards making each Silicoin service use the same default networking infrastructure and move to websockets as the default networking wire protocol.
- We added additional improvements and more RPCs to the start daemon and various services to continue to make silicoin start/stop reliable cross platform.
...
- We now codesign the Apple .dmg installer with the Silicoin Network developer ID on both GitHub Actions and Azure Pipelines. We will be notarizing and distributing the Azure Pipelines version as it's built on MacOS Mojave (10.14.6) for stronger cross version support.
...
- Starting silicoin now waits for the full node to be active before contacting the introducer.
...
- We added a `silicoin keys` command line program, to see, add, and remove private keys.
...
- We added [lgtm](https://lgtm.com/) source analysis on pull request to the silicoin-blockchain, chiapos, silicoinvdf, chiabip158, and bls-library repositories to add some automated security analysis to our ci.
...
- `silicoin netspace` has been refactored to use the `/get_network_space` RPC. The command
...
- `silicoin-check-plots` uses the plot root and checks the plots in the same manner as harvester.
- `silicoin-check-plots` now does not override plots.yaml, which means concurrent plots will properly be added to plots.yaml.
...
- We have implemented a workaround for the `silicoin start` issues some were having upon crash or reboot. We will be rebuilding start and stop to be robust across platforms.
- This release re-includes `silicoin-start-harvester`.
...
- The user interface now refers to silicoin coins with their correct currency code of XCH.
...
- Building the sub reposities (chiapos, silicoinvdf, blslibrary) now requires CMake 3.14+.

### Fixed

- There was a regression in Silicoin Proof of Space ([chiapos](https://github.com/Silicoin-Network/chiapos)) that came from our efforts to speed up plotting on Windows native. Now k>=32 plots work correctly. We made additional bug fixes and corrected limiting small k size generation.
- There was a bug in Timelord handling that could stop all VDF progress.

### Deprecated

- We have made significant changes to the full node database to make it more reliable and quicker to restart. This requires re-syncing the current chain. If you use `silicoin init` then sync on first start will happen automatically. "\$SILICOIN_ROOT" users will need to delete `$SILICOIN_ROOT/db/*` before starting Beta 1.5. This also fixes the simulation issue in Beta 1.4 where tips could go "back in time."
...
- Now you can run estimates of the total space currently farming the network. Try `silicoin netspace -d 12` to run an estimate over the last 12 blocks which is approximately 1 hour.
...
- The node RPC has a new endpoint get_header_by_height which allows you to retrieve the block header from a block height. Try `silicoin show -bh 1000` to see the block header hash of block 1000. You can then look up the block details with `silicoin show -b f655e1a9f7f8c89a703e40d9ce82ae33508badaf7b37fa1a56cad27926b5e936` which will look up a block by it's header hash.
...
- Most of the content of README.md and INSTALL.md have been moved to the [repository wiki](https://github.com/Silicoin-Network/silicoin-blockchain/wiki) and placed in [INSTALL](https://github.com/Silicoin-Network/silicoin-blockchain/wiki/INSTALL) and [Quick Start Guide](https://github.com/Silicoin-Network/silicoin-blockchain/wiki/Quick-Start-Guide)
...
- Most scripts have been removed in favor of silicoin action commands. You can run `silicoin version` or `silicoin start node` for example. Just running `silicoin` will show you more options. However `silicoin-create-plots` continues to use the hyphenated form. Also it's now `silicoin generate keys` as another example.
- Silicoin start commands like `silicoin start farmer` and `silicoin stop node` now keep track of process IDs in a run/ directory in your configuration directory. `silicoin stop` is unlikely to work on Windows native for now. If `silicoin start -r node` doesn't work you can force the run/ directory to be reset with `silicoin start -f node`.
- We suggest you take a look at our [Upgrading documentation](https://github.com/Silicoin-Network/silicoin-blockchain/wiki/Updating-beta-software) if you aren't performing a new install.
...
- The script `silicoin start wallet-gui` has been chaned to `silicoin start wallet` which launches but the GUI and server on MacOS and Linux. `silicoin start wallet-server` remains for WSL 2 and Windows native.
...
- Plots of k>=32 are not working for farming, and some broken plots can cause a memory leak. A [workaround is available](https://github.com/Silicoin-Network/silicoin-blockchain/wiki/Beta-1.4-k=32-or-larger-work-around).
...
- Currently, there is no way to restore a Coloured Coin Wallet.
- The `silicoin stop all` command sometimes fails, use `silicoin-stop-all` instead. In windows, use the task manager to stop the servers.
...
- There is a new command `silicoin init` that will migrate files and generate your initial configuration. If you want to use the Wallet or farm, you will also have to `silicoin-generate-keys`. You can read step by step instructions for [upgrading from a previous beta release](https://github.com/Silicoin-Network/silicoin-blockchain/wiki/Updating-beta-software). If you've set `$SILICOIN_ROOT` you will have to make sure your existing configuration remains compatible manually.
...
- We added full block lookup to `silicoin show`.

### Changed

- `silicoin-restart-harvester` has been renamed from `silicoin-start-harvester` to better reflect its functionality. Use it to restart a harvester that's farming so that it will pick up newly finished plots.
...
- If you previously used the plot_root variable in config, your plot directory names might not migrate correctly. Please double check the filenames in `~/.silicoin/beta-1.0b3/config/plots.yaml` after migrating
...
- There is now full transaction support on the Silicoin blockchain. In this initial Beta 1.0 release, all transaction types are supported though the wallets and UIs currently only directly support basic transactions like coinbase rewards and sending coins while paying fees. UI support for our [smart transactions](https://github.com/Silicoin-Network/wallets/blob/main/README.md) will be available in the UIs shortly.
...
- We have revamped the silicoin management command line. To start a farmer all you have to do is start the venv with `. ./activate` and then type `silicoin-start-farmer &`. The [README.md](https://github.com/Silicoin-Network/silicoin-blockchain/blob/main/README.md) has been updated to reflect the new commands.
...
- Configuration directories have been moved to a default location of HomeDirectory/.silicoin/release/config, plots/ db/, wallet/ etc. This can be overridden by `export SILICOIN_ROOT=~/.silicoin` for example which would then put the plots directory in `HomeDirectory/.silicoin/plots`.
- The libraries silicoin-pos, silicoin-fast-vdf, and silicoin-bip-158 have been moved to their own repositories: [chiapos](https://github.com/Silicoin-Network/chiapos), [silicoinvdf](https://github.com/Silicoin-Network/silicoinvdf), and [chaibip158](https://github.com/Silicoin-Network/chiabip158). They are brought in by silicoin-blockchain at install time. Our BLS signature library remains at [bls-signatures](https://github.com/Silicoin-Network/bls-signatures).
- The install process now brings in chiapos, silicoinvdf, etc from Pypi where they are auto published via GitHub Actions ci using cibuildwheel. Check out `.github/workflows/build.yml` for build methods in each of the sub repositories.
- `silicoin-regenerate-keys` has been renamed `silicoin-generate-keys`.
...
- `install-timelord.sh` has been pulled out from `install.sh`. This script downloads the source python package for silicoinvdf and compiles it locally for timelords. vdf_client can be included or excluded to make building normal user wheels easier.

### Removed

- The Beta release is not compatible with the history of the Alpha blockchain and we will be ceasing support of the Alpha chain approximately two weeks after the release of this Beta. However, your plots and keys are fully compatible with the Beta chain. Please save your plot keys! Examples of how to save your keys and upgrade to the Beta are available on the [repo wiki](https://github.com/Silicoin-Network/silicoin-blockchain/wiki).
- The ssh ui and web ui are removed in favor of the cli ui and the Electron GUI. To mimic the ssh ui try `silicoin show -s -c` and try `silicoin show --help` for usage instructions.
...
- If you want to benchmark your CPU as a VDF you can use vdf_bench square_asm 500000 for the assembly optimized test or just vdf_bench square 500000 for the plain C++ code path. This tool is found in lib/silicoinvdf/fast_vdf/.
...
- Silicoin fullnode, farmer and harvester now default to logging to silicoin.log in the silicoin-blockchain directory. This is configured in config.yaml and due to config.yaml changes it is recommended to edit the new template config instead of using older config.yaml’s from previous versions.
...
- VDF improvements - VDF verification and generation speed has increased and dependence on flint2 has been removed. We wish to thank Dr. William Hart (@wbhart) for dual licensing parts of his contributions in FLINT and Antic for inclusion in the Silicoin blockchain.
...
- Added color logging
- Now silicoin_harvester will periodically announce which plots it is currently farming and their k sizes.

### Changed

- Moved the ssh UI to use RPC.
- Changed the displayed process names for harvester, farmer, fullnode, timelords, and VDFs to to silicoin_full node, silicoin_harvester, etc. Fixed a bug that could cause inadvertent shutdown of other processes like an ongoing plotting session when new silicoin services were started.
...
- This is the first release of the Silicoin testnet! Blockchain consensus, proof of time, and proof of space are included.
- More details on the release at [https://www.silicoin.net/developer/](https://www.silicoin.net/developer/)

[unreleased]: https://github.com/Silicoin-Network/silicoin-blockchain/compare/1.0beta5...dev
[1.0beta5]: https://github.com/Silicoin-Network/silicoin-blockchain/compare/1.0beta4...1.0beta5
[1.0beta4]: https://github.com/Silicoin-Network/silicoin-blockchain/compare/1.0beta3...1.0beta4
[1.0beta3]: https://github.com/Silicoin-Network/silicoin-blockchain/compare/1.0beta2...1.0beta3
[1.0beta2]: https://github.com/Silicoin-Network/silicoin-blockchain/compare/1.0beta1...1.0beta2
[1.0beta1]: https://github.com/Silicoin-Network/silicoin-blockchain/compare/alpha-1.5.1...1.0beta1
[alpha 1.5.1]: https://github.com/Silicoin-Network/silicoin-blockchain/compare/alpha-1.5...alpha-1.5.1
[alpha 1.5]: https://github.com/Silicoin-Network/silicoin-blockchain/compare/alpha-1.4.1...alpha-1.5
[alpha 1.4.1]: https://github.com/Silicoin-Network/silicoin-blockchain/compare/alpha-1.4...alpha-1.4.1
[alpha 1.4]: https://github.com/Silicoin-Network/silicoin-blockchain/compare/alpha-1.3...alpha-1.4
[alpha 1.3]: https://github.com/Silicoin-Network/silicoin-blockchain/compare/alpha-1.2...alpha-1.3
[alpha 1.2]: https://github.com/Silicoin-Network/silicoin-blockchain/compare/alpha-1.1.1...alpha-1.2
[alpha 1.1.1]: https://github.com/Silicoin-Network/silicoin-blockchain/compare/alpha-1.1...alpha-1.1.1
[alpha 1.1]: https://github.com/Silicoin-Network/silicoin-blockchain/compare/alpha-1.0...alpha-1.1
[alpha 1.0]: https://github.com/Silicoin-Network/silicoin-blockchain/releases/tag/Alpha-1.0
