...
from silicoin.protocols import full_node_protocol, introducer_protocol
from silicoin.protocols.protocol_message_types import ProtocolMessageTypes
from silicoin.server.address_manager import AddressManager, ExtendedPeerInfo
from silicoin.server.address_manager_store import AddressManagerStore
from silicoin.server.outbound_message import NodeType, make_msg
from silicoin.server.server import SilicoinServer
from silicoin.types.peer_info import PeerInfo, TimestampedPeerInfo
...
class FullNodeDiscovery:
    ...
    def __init__(
        ...
        self.server: SilicoinServer = server
    ...
    async def on_connect(self, peer: ws.WSSilicoinConnection):
        if (
    ...
    async def update_peer_timestamp_on_message(self, peer: ws.WSSilicoinConnection):
        if (
            peer.is_outbound
            and peer.peer_server_port is not None
            and peer.connection_type is NodeType.FULL_NODE
    ...
    async def _introducer_client(self):
        ...
        async def on_connect(peer: ws.WSSilicoinConnection):
    ...
    async def on_connect_callback(self, peer: ws.WSSilicoinConnection):
