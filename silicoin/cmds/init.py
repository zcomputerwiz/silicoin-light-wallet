...
@click.option("--testnet", is_flag=True, help="Configure this silicoin install to connect to the testnet")
...
def init_cmd(ctx: click.Context, create_certs: str, fix_ssl_permissions: bool, testnet: bool, **kwargs):
    ...
    - Make a copy of your Farming Machine CA directory: ~/.silicoin/[version]/config/ssl/ca
    - Shut down all silicoin daemon processes with `silicoin stop all -d`
    ...
    from silicoin.cmds.passphrase_funcs import initialize_passphrase
...
if __name__ == "__main__":
    from .init_funcs import silicoin_init
    from silicoin.util.default_root import DEFAULT_ROOT_PATH

    silicoin_init(DEFAULT_ROOT_PATH)
