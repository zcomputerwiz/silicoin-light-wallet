...
from silicoin.util.misc import format_bytes, format_minutes
from silicoin.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total silicoin farmed: {amounts['farmed_amount'] / units['silicoin']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['silicoin']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['silicoin']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print(
                "For details on farmed rewards and fees you should run 'silicoin start wallet' and 'silicoin wallet show'"
            )
