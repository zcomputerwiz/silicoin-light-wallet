...
from silicoin.wallet.trading.offer import Offer, NotarizedPayment
...
class TestOfferLifecycle:
    ...
    async def test_complex_offer(self, setup_sim):
        ...
        try:
            ...
            silicoin_coins: List[Coin] = all_coins[None]
            ...
            silicoin_requested_payments: Dict[Optional[bytes32], List[NotarizedPayment]] = Offer.notarize_payments(
                silicoin_requested_payments, silicoin_coins
            )
            silicoin_announcements: List[Announcement] = Offer.calculate_announcements(silicoin_requested_payments)
            silicoin_secured_bundle: SpendBundle = self.generate_secure_bundle(silicoin_coins, silicoin_announcements, 1000)
            ...
            new_offer = Offer.aggregate([silicoin_offer, red_offer])
