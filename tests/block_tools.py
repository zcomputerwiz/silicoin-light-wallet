...
from silicoin.cmds.init_funcs import create_all_ssl, create_default_silicoin_config
from silicoin.daemon.keychain_proxy import connect_to_keychain_and_validate, wrap_local_keychain
from silicoin.full_node.bundle_tools import (
    best_solution_generator_from_template,
    detect_potential_template_generator,
    simple_solution_generator,
)
from silicoin.util.errors import Err
from silicoin.full_node.generator import setup_generator_args
from silicoin.full_node.mempool_check_conditions import GENERATOR_MOD
from silicoin.plotting.create_plots import create_plots, PlotKeys
...
from silicoin.consensus.coinbase import create_puzzlehash_for_pk, create_farmer_coin, create_pool_coin
from silicoin.consensus.condition_costs import ConditionCost
...
from silicoin.plotting.util import PlotsRefreshParameter, PlotRefreshResult, PlotRefreshEvents, parse_plot_info
...
from silicoin.types.blockchain_format.reward_chain_block import RewardChainBlockUnfinished
...
from silicoin.types.generator_types import BlockGenerator, CompressorArg
...
from silicoin.util.ints import uint8, uint16, uint32, uint64, uint128
from silicoin.util.keychain import Keychain, bytes_to_mnemonic
...
from silicoin.util.vdf_prover import get_vdf_info_and_proof
    ...
    async def setup_keys(self):
        if self.local_keychain:
            ...
            self.keychain_proxy = await connect_to_keychain_and_validate(
                self.root_path, log, user="testing-1.8.0", service="silicoin-testing-1.8.0"
            )
        ...
        if len(self.pool_pubkeys) == 0 or len(self.farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `silicoin generate keys`")
...
def get_plot_dir() -> Path:
    cache_path = Path(os.path.expanduser(os.getenv("SILICOIN_ROOT", "~/.silicoin/"))) / "test-plots"
    mkdir(cache_path)
    return cache_path
