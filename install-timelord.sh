...
THE_PATH=$(python -c 'import pkg_resources; print( pkg_resources.get_distribution("silicoinvdf").location)' 2>/dev/null)/vdf_client
SILICOINVDF_VERSION=$(python -c 'from setup import dependencies; t = [_ for _ in dependencies if _.startswith("silicoinvdf")][0]; print(t)')
...
if [ -e "$THE_PATH" ]; then
	...
	if [ -e venv/bin/python ] && test $UBUNTU_DEBIAN; then
		echo "Installing silicoinvdf from source on Ubuntu/Debian"
		...
		echo venv/bin/python -m pip install --force --no-binary silicoinvdf "$SILICOINVDF_VERSION"
		venv/bin/python -m pip install --force --no-binary silicoinvdf "$SILICOINVDF_VERSION"
		symlink_vdf_bench "$PYTHON_VERSION"
	elif [ -e venv/bin/python ] && test $RHEL_BASED; then
		echo "Installing silicoinvdf from source on RedHat/CentOS/Fedora"
		# Install remaining needed development tools - assumes venv and prior run of install.sh
		...
		echo "Installing silicoinvdf requirement boost for MacOS."
		brew install boost
		echo "Installing silicoinvdf from source."
